...
import com.example.aboutme._core.utils.UserUtil;
    ...
    public User loginKakao(String code, HttpSession session) {
        ...
        if (userPS != null) {
            ...
            User user = User.builder()
                    .name(nickname)
                    .password(UUID.randomUUID().toString()) // 비밀번호는 임시로 생성
                    .email(email != null ? email : "kakao_" + kakaoId + "@kakao.com")
                    .phone("000-0000-0000")
                    .userRole(userRole) // 사용자가 선택한 역할로 설정
                    .profileImage(UserUtil.getDefaultProfileImage()) // 기본 프로필 이미지 설정
